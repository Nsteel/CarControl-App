 ...
 * Shows the robot's speed and direction, and signal strength.
...
public class HUDFragment extends SimpleFragment implements MessageListener<CarTelemetryWrapper>{
    ...
    private ImageView wifiStrengthView, batterySystemView, batteryMotorView;
    ...
    // Icons for indicating Battery charge level
...
//          locationView = (TextView) view.findViewById(R.id.hud_location);
     ...
     */
    void updateUI(final double speed, final double turnrate, final double batterySystemVoltage, final double batteryMotorVoltage)
    {
        if (!isDetached()) {
            lastSpeed = speed;
    ...
    /**
     * Callback for receiving OdomSensroWrapper messages.
    ...
    private class UpdateUIRunnable implements Runnable
    {
        ...
        public void run() {
            ...
            try {
                ...
                //if (speedView != null)
                  //  speedView.setText(String.format((String) getText(R.string.percent_string), 100));
